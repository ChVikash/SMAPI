...
async def createposts( db:Session = Depends(get_db),  current_user: models.User = Depends(oauth2.get_current_user) ):
    ...
    # print(val)
    row_inserted = models.Post(user_id = current_user.id, **(user_post.model_dump()))
    db.add(row_inserted)
    db.commit()
...
async def posts(db : Session = Depends(get_db), user = Depends(oauth2.get_current_user), limit: int =15, skip: int = 0, search: Optional[str]= ""):
    rows = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit=limit).offset(skip).all()
    return rows
...
def updatepostbyid(id: int, post :schemas.PostCreateUpdate, db:Session = Depends(get_db),  current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    if updated_post.user_id != current_user.id : 
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Action can't be performed")
